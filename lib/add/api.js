...
class AddAPI extends FileController {
    ...
    injectVue(file, options = {}) {
        ...
        if (options.template) {
            (Array.isArray(options.template) ? options.template : [options.template]).forEach(
                imp => {
    ...
    injectVueStore(file, imports) {
        let _imports =
            this.addor.injectVueStore[file] || (this.addor.injectVueStore[file] = new Set());
        (Array.isArray(imports) ? imports : [imports]).forEach(imp => {
            _imports.add(imp);
        });
    }
