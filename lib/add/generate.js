...
class Addor extends EventEmitter {
    ...
    async create() {
        ...
        let options = await this.resolveMeta();
    ...
    async resolveMeta() {
        ...
        let cacheData = db.get("prompts").find({ cwd: this.projectPath, id: this.baseDir }).value();
        let needCaches = [], needObj = {};
        this.meta.prompts.forEach((p, i) => {
            if (cacheData && p.name in cacheData) {
                this.meta.prompts.splice(i, 1);
            } else {
                if (p.cache) {
        ...
        if (this.meta.prompts.length>0) {
            ...
            if (needCaches.length>0) {
                ...
                if (cacheData) {
                    db.get("prompts").find({ cwd: this.projectPath, id: this.baseDir }).assign(needObj).write();
                } else {
                    db.get("prompts").push(Object.assign({ 
                        id: this.baseDir, 
        ...
        options.meta = Object.assign(options.meta, db.get("prompts").find({ cwd: this.projectPath, id: this.baseDir }).value());
